(function(root, factory) {
	...
	var buildCache = function(subtitles) {
		...
		for(var j = 0; j < subtitles.length; j++) {
			...
			var cacheNames = getCacheNames(
				subtitle.getAttribute("data-timestamp-start"), 
				subtitle.getAttribute("data-timestamp-end")
			);
			
			for (var k = 0; k < cacheNames.length; k++) {
				if (!cache[cacheNames[k]]) {
					cache[cacheNames[k]] = [];
				}
				
				cache[cacheNames[k]].push(subtitle);
			}
		}
	...
	var firstInit = function() {
		...
		style.type      = 'text/css';
		style.innerHTML = ".youtube-external-subtitle { position: absolute; display: none; z-index: 0; pointer-events: none; color: yellow; font-family:Verdana, Geneva, sans-serif; font-size: 14px; text-align: center; font-weight: bold; text-shadow: -1px 0 black, 0 1px black, 1px 0 black, 0 -1px black;} .youtube-external-subtitle span { background: rgba(0, 0, 0, 0.5); padding: 1px 4px; display: inline-block; margin-bottom: 2px; } .youtube-external-subtitle.fullscreen-ignore { display: none !important; } .youtube-external-subtitle.fullscreen { z-index: 3000000000; }";
	...
	Subtitle.prototype.getSubtitleFromCache = function(seconds) {
		...
		for (var i in cache) {
			if (seconds >= cache[i].getAttribute("data-timestamp-start") 
			&& seconds <= cache[i].getAttribute("data-timestamp-end")) {
				return cache[i];
			}
		}
	...
	Subtitle.prototype.render = function() {
		if (this.subtitle === null) {
			...
			this.element.innerHTML = '<span>' + this.subtitle.innerHTML.replace(/(?:\r\n|\r|\n)/g, '</span><br /><span>') + '</span>';
